...
namespace Nano {
	namespace Communication {
		...
		inline bool SessionManager::addSession(std::shared_ptr<Session> session)
		{
			...
			ASYNC_LOG_INFO(ASYNC_LOG_NAME("STD_LOGGER"), "SessionManager") << "Connected: " << session->getUid() << std::endl;
			return true;
		}
		...
		inline void SessionManager::removeSession(std::string& uid)
		{
			...
			ASYNC_LOG_INFO(ASYNC_LOG_NAME("STD_LOGGER"), "SessionManager") << "Disconnected: " << uid << std::endl;
		}
	}
}